...
public class RegAdd {
	 ...
	 * 在注册表里面添加空项
	 * 
	 * @param primaryKey 要操作的注册表主键，值位于com.gitee.swsk33.reg.utils.RegPrimaryKey类中（例如RegPrimaryKey.HKCR即为HKEY_CLASSES_ROOT）
	 * @param name       项名称
	 ...
	 * @param primaryKey 要操作的注册表主键，值位于com.gitee.swsk33.reg.utils.RegPrimaryKey类中（例如RegPrimaryKey.HKCR即为HKEY_CLASSES_ROOT）
	 * @param name       项名称
	 * @param type       默认值的类型，值位于com.gitee.swsk33.reg.utils.RegDataType类中（例如RegDataType.REG_SZ即为REG_SZ字符串数据）
	 * @param data       默认值的值
	 * @return 布尔值 添加成功返回true
	 * @throws Exception 没有管理员权限时抛出异常
	 */
	 ...
	 * @param primaryKey 要操作的注册表主键，值位于com.gitee.swsk33.reg.utils.RegPrimaryKey类中（例如RegPrimaryKey.HKCR即为HKEY_CLASSES_ROOT）
	 * @param name       项名称
	 * @param type       值的类型，值位于com.gitee.swsk33.reg.utils.RegDataType类中（例如RegDataType.REG_SZ即为REG_SZ字符串数据）
